...
class TeamShareAdder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        db_reader = DBReader(
            self.db_config['address'], self.db_config['username'], self.db_config['password'])
        player_stat_names, team_stat_names = list(zip(*self.config))
        player_stats_indices = [element[0] for element in enumerate(
            self.attribute_names) if element[1] in player_stat_names]
        team_stats_indices = [element[0] for element in enumerate(
            self.attribute_names) if element[1] in team_stat_names]
